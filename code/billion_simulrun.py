...
def main():
    ...
    dataset = h5py.File(DATASETS[DATASET]['filepath'], 'r')['train']
    ...
    buildGraph = billionRobustPrune

    for source in sources_to_process[:1]:
        if source not in completed:

            edges = buildGraph(source, dataset, 10000000)
